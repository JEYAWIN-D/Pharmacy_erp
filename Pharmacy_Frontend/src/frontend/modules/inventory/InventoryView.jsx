      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="unique-card p-6 lg:col-span-2 text-left space-y-4">
          <h4 className="text-xs font-extrabold text-slate-800 uppercase tracking-wider flex items-center gap-2">
            ...
            Live Warehouse vs Rack Stock Levels
          </h4>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 uppercase font-bold text-[10px]">
                  ...
                  <th className="py-2.5 text-center">Warehouse Stock</th>
                  <th className="py-2.5 text-center">Rack Compartment Stock</th>
                  <th className="py-2.5 text-center font-bold">Total Stock</th>
                  <th className="py-2.5 text-center">Stock Status</th>
                </tr>
              </thead>
              <tbody>
                {medicines.map(med => {
                  const whQty = med.inventory?.warehouseStock ?? 0;
                  const rackQty = med.inventory?.rackStock ?? 0;
                  const totalQty = med.inventory?.totalStock ?? med.stock ?? 0;
                  ...
                  return (
                    <tr key={med.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="py-3 text-center text-slate-600 font-semibold">{whQty} pcs</td>
                      <td className="py-3 text-center text-slate-600 font-semibold">{rackQty} pcs</td>
                      <td className="py-3 text-center font-black text-slate-800">{totalQty} pcs</td>
                      <td className="py-3 text-center">
                        {isLow ? (
                          <span className="px-2 py-0.5 rounded text-[9px] font-bold text-red-700 bg-red-50">Low Stock!</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded text-[9px] font-bold text-emerald-700 bg-emerald-50">Fully Stocked</span>
                        )}
                      </td>
                    </tr>
                  );
